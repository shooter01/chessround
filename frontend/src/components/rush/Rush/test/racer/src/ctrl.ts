...
import type { PuzCtrl, PuzPrefs, Run } from './lib/puz/interfaces';
...
export default class RacerCtrl implements PuzCtrl {
  ...
  constructor(
    ...
    this.run = {
      ...
      // clock: new Clock(
      //   config,
      //   defined(opts.data.startsIn) ? Math.max(0, -opts.data.startsIn) : undefined,
      // ),
      history: [],
  ...
  isPlayer = () =>
    !this.vm.alreadyStarted && this.data.players.some((p) => p.name === this.data.player.name);

  raceFull = () => this.data.players.length >= 10;

  status = (): RaceStatus =>
    this.run.clock.started() ? (this.run.clock.flag() ? 'post' : 'racing') : 'pre';
